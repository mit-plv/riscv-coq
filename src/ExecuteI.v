  ...
  fun arg_0__ =>
    ...
    | Decode.Lb rd rs1 oimm12 =>
        Bind (getRegister rs1) (fun a =>
                Bind (translate Load one (a + fromImm oimm12)) (fun addr =>
                        Bind (loadByte addr) (fun x => setRegister rd (int8ToReg x))))
    | Decode.Lh rd rs1 oimm12 =>
        Bind (getRegister rs1) (fun a =>
                Bind (translate Load two (a + fromImm oimm12)) (fun addr =>
    ...
    | Decode.Slti rd rs1 imm12 =>
        Bind (getRegister rs1) (fun x =>
                setRegister rd (if x < fromImm imm12 : bool then one else zero))
    | Decode.Sltiu rd rs1 imm12 =>
        Bind (getRegister rs1) (fun x =>
                setRegister rd (if (ltu x (fromImm imm12)) : bool then one else zero))
    ...
    | Decode.Slt rd rs1 rs2 =>
        Bind (getRegister rs1) (fun x =>
                Bind (getRegister rs2) (fun y =>
                        setRegister rd (if x < y : bool then one else zero)))
    | Decode.Sltu rd rs1 rs2 =>
        Bind (getRegister rs1) (fun x =>
                Bind (getRegister rs2) (fun y =>
                        setRegister rd (if (ltu x y) : bool then one else zero)))
